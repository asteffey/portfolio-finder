...
allocations = pf.create_portfolio_allocations(0.05, ['USA_TSM', 'WLDx_TSM', 'USA_INT', 'USA_BILL', 'USA_REIT', 'GLD', 'EM', 'USA_SCB'])
allocations = list(filter(lambda x: x.USA_TSM>=0.25 and x.WLDx_TSM<=0.4 and x.USA_INT>=0.1 and x.USA_INT<=0.3 and x.USA_BILL<=0.2 and x.USA_REIT<=0.2 and x.GLD<=0.2 and x.EM<=0.2 and x.USA_SCB<=0.4, allocations))
